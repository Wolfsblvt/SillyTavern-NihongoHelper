...
Higher-order feedback on Japanese the **user** writes (distinct from the mechanical tooltip lookup). One shared engine, two entry points, opt-in automatic mode. Six modules, plus a refactor of `side-chat-llm.js`. Feedback prose is **English**; Japanese is reserved for the learner's own language content, and all text renders through the same pipeline as the main chat (markdown + furigana + tooltips).
...
**`src/feedback-render.js`** — Shared DOM. `createFeedbackCard({ mode: 'attached'|'modal', expanded, showApply, callbacks })` is a small state machine (`setLoading`/`setError`/`setResult`/`setStale`) with collapsed/expanded header (badges: issue count + highest severity + loading/error/stale). `buildResultView` / `buildIssueCard` are exported for the modal. **Model text is rendered through `messageFormatting`** (markdown + the `afterMarkdown` furigana hook + DOMPurify) via the local `renderFormatted` helper, so feedback gets the same automatic furigana and inspect-mode word/kanji tooltips as the chat; inline fields (quote/replacement/alternatives/strengths) unwrap a lone `<p>` so ruby flows inline. DOMPurify (run last by `messageFormatting`) is the sanitization boundary for untrusted model output. A **collapsible reasoning block** sits at the top of the body: expanded and streaming during analysis (`setLoading(reasoning)` updates it in place + auto-scrolls), auto-collapsed but still expandable once the parsed result renders.
...
- *Inline correction overlay:* render anchored issues as underlines/highlights directly over the message text (anchors already carry exact offsets) with a per-span popover — the anchor model was designed with this in mind.
