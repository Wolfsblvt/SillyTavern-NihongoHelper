...
Higher-order feedback on Japanese the **user** writes (distinct from the mechanical tooltip lookup). One shared engine, two entry points, opt-in automatic mode, plus an inline anchored overlay with reversible staging on sent messages. Seven modules, plus a refactor of `side-chat-llm.js`. Feedback prose is **English**; Japanese is reserved for the learner's own language content, and all text renders through the same pipeline as the main chat (markdown + furigana + tooltips).
...
- `applyStagedFixes(source, issues)` → applies a set of staged issue replacements to the source in **one pass, right-to-left by resolved offset** so earlier edits don't shift later ones; silently skips unsafe/unlocatable anchors and **overlap-detects** (a later span intersecting an already-applied one is dropped). Returns `{ text, applied, skipped }`. Pure and unit-tested — it underpins the staging preview + commit.
- `hashText()` (FNV-1a, whitespace-normalized) for source association / staleness, `hasJapaneseContent()` heuristic, and `DEFAULT_GLOBAL_FEEDBACK_INSTRUCTIONS` (editable prose; the JSON contract is assembled in code, not here). The default instructions mandate **English** for `summary`/explanations and Japanese only for quotes/replacements/revised text, and include a learner-pitfall checklist (pronoun overuse, は/が, politeness consistency, transitive/intransitive, 和製英語). Category **descriptions** are tuned for learners; category **ids are stable** (they appear in persisted records).
...
**`src/feedback-engine.js`** — `runFeedback({ targetText, beforeIndex, requestKey, controller, onReasoning })` builds prompts → one `requestCompletion` call → buffers the **final** response (no partial-JSON parsing) → parses/validates → resolves anchors. Returns `{ ok, result?, error?, aborted?, raw?, reasoning?, meta }` — the final provider reasoning is surfaced in the outcome (and persisted). Holds an **in-flight registry** keyed by `requestKey` (`buildRequestKey(chatId, text)`) so callers can dedupe, skip, or abort. Reasoning is also streamed transiently via `onReasoning`; only the final content is parsed.

**`src/feedback-render.js`** — Shared DOM. `createFeedbackCard({ mode: 'attached'|'modal', expanded, showApply, callbacks })` is a small state machine (`setLoading`/`setError`/`setResult`/`setStale`) with collapsed/expanded header (badges: issue count + highest severity + loading/error/stale). `buildResultView` / `buildIssueCard` / `buildStageToggle` (and `renderFormatted`) are exported for reuse by the modal and overlay. **Model text is rendered through `messageFormatting`** (markdown + the `afterMarkdown` furigana hook + DOMPurify) via the local `renderFormatted` helper, so feedback gets the same automatic furigana and inspect-mode word/kanji tooltips as the chat; inline fields (quote/replacement/alternatives/strengths) unwrap a lone `<p>` so ruby flows inline. DOMPurify (run last by `messageFormatting`) is the sanitization boundary for untrusted model output. A **collapsible reasoning block** sits at the top of the body: expanded and streaming during analysis (`setLoading(reasoning)` updates it in place + auto-scrolls), auto-collapsed but still expandable once the parsed result renders. When `setResult` is given a **session** (see overlay), the card subscribes to it and re-renders reactively: a header **inline-highlights toggle** (eye/marker, hidden in `always` mode), per-issue **Stage fix** toggles, a **Stage revision** toggle, and a **staging tray** (status + Apply/Clear + a live `messageFormatting` preview of the resulting text).

**`src/feedback-messages.js`** — Sent-message entry point. Injects a `.mes_nihongo_feedback` button into `#message_template .extraMesButtons` (cloned onto new messages) + existing messages; CSS hides it on non-user messages. `runFeedbackForMessage(id, { regenerate, auto })` persists results to **`message.extra.nihongoFeedback`** (custom key → never enters the prompt) and attaches the card at the end of `.mes_block`. Lifecycle hooks (`CHAT_CHANGED`, `MORE_MESSAGES_LOADED`, `MESSAGE_UPDATED/EDITED/SWIPED/DELETED`, `USER_MESSAGE_RENDERED`) re-attach cards, recompute **staleness** (current text hash vs stored `sourceHash`), and clean up. **Automatic mode** runs on `USER_MESSAGE_RENDERED`, detached (never awaited → never blocks main generation), gated by `hasJapaneseContent`, deduped by hash. **Chat-switch/deletion safety:** a late result is validated against `getCurrentChatId()` and `chat.indexOf(message)` before it ever persists or attaches. Each rendered card gets a **session** (`makeSession`) wiring overlay state to settings: `applyAllowed` (from `feedbackApplyPolicy` + position, and never when stale), `inlineToggleable`/`initialInlineVisible` (from `feedbackInlineMode` — `auto` shows on the latest user message, i.e. no newer user message follows). Committing staged text edits the message in place (`updateMessageBlock` → `saveChatConditional` → emit `MESSAGE_UPDATED`), which re-renders furigana and marks the feedback stale. A `nihongo-feedback-inline-mode-changed` DOM event (dispatched by settings) refreshes all cards.

**`src/feedback-draft.js`** — Draft entry point. Adds `#nihongo_review_button` to `#leftSendForm`. Opens a blocking `Popup` (`okButton: 'Apply to composer'`, `cancelButton: 'Close'`) hosting the shared card (modal mode, `showApply`) plus an editable working copy. Applying a revision / single fix writes into the **working copy** only; the composer is touched **only** on `AFFIRMATIVE`. Closing aborts the request (`onClosing`). Per-issue apply re-resolves the quote against the *current* working text and bails when ambiguous. The dialog gets its own `attachKanjiTooltip` (appended to the dialog so it layers above the popup) for the same hover tooltips as the chat, detached on close.

**`src/feedback-overlay.js`** — Inline anchored overlay + the per-message **session**. `createFeedbackSession({ result, getSourceText, applyAllowed, inlineToggleable, initialInlineVisible, onCommit })` is a tiny pub/sub holding the reversible **staging** state (individual fixes via a `Set`, or the whole revision — mutually exclusive), the inline-visibility flag, a `previewText()` (delegates to `applyStagedFixes`, or `result.revisedText` when the full revision is staged), and `commit()`. `applyOverlayState(mesEl, session, { stale })` reconciles the marks: render when visible & fresh, **update staged styling in place** when they already exist (so an open popover survives), or clear. `renderInlineMarks` maps each issue quote to a DOM range by **searching the rendered `.mes_text` while ignoring ruby `<rt>`/`<rp>`** (a `TreeWalker` text map + occurrence-aware `indexOf`), then wraps the covered text-node segments (splitting at boundaries) — never trusting the source-offset anchors, which don't line up with markdown-transformed, furigana-injected DOM. **Overlapping quotes are kept, not dropped** (a broad particle fix containing a narrow orthography fix is common): the union of ranges is split at every boundary into maximal sub-segments, each carrying its set of covering issues. Each mark draws **one stacked underline per covering issue** (layered `background-image` gradients, colours from a single `MARK_SEV_COLOR` map): the longest/outer issue is the top line — continuous across the phrase because a constant `padding-bottom` keeps it aligned across segments — and nested shorter notes stack below it in their own severity colours (capped, with the rest still in the popover). A **singleton click popover** (not hover — marks contain `.nihongo-word` spans that would fight inspect-mode tooltips) shows category/severity/quote→replacement/explanation + a Stage toggle per issue (a count header + one block per issue when a mark covers several), re-renders on session change, dismisses on outside-click/Esc, and calls `hideKanjiTooltip()` on open.
...
**Persisted record** (`message.extra.nihongoFeedback`): `{ v, sourceHash, createdAt, presetId, presetName, sensitivity, reasoning, result }`. The model's **reasoning is persisted** (shown collapsed on re-render); the raw text dump is not.

**Settings:** `feedbackAutoMode` (`off`/`japanese`), `feedbackContextCount` (0–10, default 4), `feedbackSensitivity` (default `balanced`), `feedbackExpandedByDefault`, `feedbackGlobalInstructions` (`''` = bundled default; reset button), `feedbackInlineMode` (`off`/`auto`/`always`, default `off` — inline highlights on sent messages), `feedbackApplyPolicy` (`latest-only`/`one-reply`/`always`, default `one-reply` — when in-place apply is offered, by how many messages follow). Tutor presets gained an optional top-level `feedback` string (style only) — passed through `migrateToCurrent`, read via `getActivePresetFeedbackGuidance()`.

**Tests:** `scripts/test-feedback.mjs` (run `node scripts/test-feedback.mjs`) — pure-logic assertions covering malformed/fenced parsing, validation/clamping, repeated-quote anchoring, apply-safety, severity ordering, hashing, and the JP heuristic. The rendering/reasoning/tooltip paths are DOM/browser-only and not covered here.

**Category design rationale.** The 10 substantive categories (+`other`) map to the axes a learner actually wants separated: `particle` and `conjugation` are split out from `grammar` because they are the highest-frequency, most-correctable learner errors; `naturalness` vs `meaning` vs `word_choice` separate "unnatural-but-correct" from "wrong meaning" from "wrong word"; `register` and `context` cover social/discourse fit; `orthography` and `punctuation` cover surface form. Because ids live in the persisted record they are treated as a **stable API** — wording/descriptions can evolve freely, but renaming an id would orphan old feedback (it falls back to `other`).

**Inline overlay & staging — design rationale.** The overlay deliberately **does not reuse the stored source-offset anchors** for placement: the rendered `.mes_text` is markdown-transformed and carries furigana `<rt>` text that isn't in the source, so offsets won't line up. Instead it re-locates each quote by occurrence-aware search over a ruby-stripped text map — the same skip-`<rt>` strategy the kanji tooltip walk uses — and wraps the matching text nodes. The popover is **click**-triggered because a mark's interior `.nihongo-word` spans already own hover for inspect-mode tooltips. Applying is a **staging** model rather than per-button immediate edits: multiple fixes are reversible, previewed live, then committed in **one** message edit, and `applyStagedFixes` works **right-to-left with overlap detection** so combined edits don't corrupt each other. Two settings keep it unobtrusive and safe: `feedbackInlineMode` controls when marks appear (with a per-message toggle as an ephemeral override), and `feedbackApplyPolicy` hides Apply once the message is "buried" (older than the latest exchange) so you don't silently rewrite history mid-conversation. Staging is also disabled outright when feedback is stale, since the anchors no longer match the text.

**Future ideas (not yet built).**
- *Per-category enable/disable & JLPT targeting:* let the learner mute categories (e.g. `punctuation`) or bias explanation depth toward a target JLPT level; the category registry + the learner-level prompt line are the hooks.
- *Continuous draft review:* debounced re-analysis as the user types (the in-flight registry + abort already support rapid re-runs).
- *Feedback history / progress view:* aggregate persisted records into a "common mistakes" summary that feeds back into the learner model.
- *Collocation / nuance split:* if `word_choice` proves too broad in practice, split out collocation and near-synonym nuance as their own categories.
